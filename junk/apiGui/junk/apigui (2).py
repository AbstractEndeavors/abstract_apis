...
import threading  # Optional, but useful for any non-Qt threading if needed
...
class RequestThread(QThread):
    response_signal = pyqtSignal(str, str)  # Emits response text and log message
    error_signal = pyqtSignal(str)  # Emits error message
    log_signal = pyqtSignal(str)  # For logging from thread

    def __init__(self, method: str, url: str, headers: dict, params: dict, is_fetch_endpoints: bool = False, is_detect: bool = False):
        super().__init__()
        self.method = method
        self.url = url
        self.headers = headers
        self.params = params
        self.is_fetch_endpoints = is_fetch_endpoints  # Flag for special handling in fetch_remote_endpoints
        self.is_detect = is_detect  # Flag for detect_api_prefix

    def run(self):
        try:
            if self.is_detect:
                # Special handling for detect_api_prefix (uses requests directly)
                candidates = [f"{self.url}/config", f"{self.url}/__config", f"{self.url}/_meta"]
                found: Optional[str] = None
                for cand_url in candidates:
                    r = requests.get(cand_url, timeout=3)
                    if r.ok:
                        j = r.json()
                        ...
                        if isinstance(val, str) and val.strip():
                            ...
                            break
                if found:
                    self.response_signal.emit(found, "API prefix set to: " + found)
                else:
                    raise Exception("No valid API prefix found in config endpoints")
            else:
                # Normal GET/POST
                if self.method == "GET":
                ...
                if self.is_fetch_endpoints:
                    # Special handling for fetch_remote_endpoints
                    if isinstance(res, list):
                        txt = json.dumps(res)  # We'll parse it in the main thread
                        self.response_signal.emit(txt, "✔ Remote endpoints loaded")
                    else:
                        raise Exception(f"/endpoints returned non-list, ignoring")
                else:
                    # Standard send_request response
...
class APIConsole(QWidget):
    ...
    def fetch_remote_endpoints(self):
        ...
        self.fetch_button.setEnabled(False)  # Prevent multiple clicks
        thread = RequestThread("GET", url, {}, {}, is_fetch_endpoints=True)
        thread.response_signal.connect(self._on_fetch_response)
        thread.error_signal.connect(self._on_fetch_error)
        thread.finished.connect(lambda: self.fetch_button.setEnabled(True))
        thread.start()
    ...
    def send_request(self):
        ...
        thread = RequestThread(method, url, headers, params)
        thread.response_signal.connect(self._on_send_response)
        thread.error_signal.connect(self._on_send_error)
        thread.finished.connect(lambda: self.send_button.setEnabled(True))
        thread.start()
    ...
    def detect_api_prefix(self):
        base = self.base_combo.currentText().rstrip("/")
        # Note: We pass base as url, since candidates are built in the thread
    ...
    def _on_detect_error(self, err: str):
        logging.error(err)
        QMessageBox.warning(self, "Detect Error", "Failed to detect API prefix: " + err)
        self.api_prefix = "/api"
        self.api_prefix_in.setText(self.api_prefix)
